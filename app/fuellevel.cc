...
namespace {
    constexpr uint16_t map_range(uint16_t value, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max) {
    ...
    constexpr uint16_t tankSensorToMl(uint16_t tank_sensor_adc) {
        ...
        return map_range(tank_sensor_adc, EMPTY_TANK_ADC, FULL_TANK_ADC, 0, FULL_TANK_ML);
    }
}
